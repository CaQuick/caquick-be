...
**표시값 스냅샷.** 다른 도메인의 표시값(주문 품목의 매장명·상품 썸네일, 리뷰의 주문 품목 표시값, 알림의 본문)은 조회 시 조인이 아니라 **생성 시점에 복사한 컬럼**을 읽는다. 원본이 바뀌어도 주문·리뷰 화면은 그때 값을 유지해야 하고(도메인 요구), 조회 경로가 다른 feature의 테이블에 묶이지 않는다.
