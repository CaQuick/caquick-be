...
| **경보 (선택)**     | `DISCORD_ALERT_WEBHOOK_URL` — outbox FAILED·부팅 실패 등 운영 경보를 보낼 Discord 웹훅. 미설정이면 로그로만 남긴다. `ALERT_DEDUPE_WINDOW_MS`(기본 300000) — 같은 경보 억제 창. `BOOT_ALERT_STATE_DIR` — 부팅 실패 경보 억제 파일 위치(기본 `~/.caquick/boot-alert`, 700)                                                                                                                                                     |
