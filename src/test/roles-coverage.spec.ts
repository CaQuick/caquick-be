...
// 행위자 feature(seller·admin)가 도메인 feature로 흩어진 뒤에도 접두 루트 필드 전수가 역할 가드를 갖추는지 본다.
...
describe('접두 루트 필드 인가 커버리지', () => {
  ...
  describe.each([
    ...
    it.each(fields)('%s 핸들러는 RolesGuard + @Roles를 갖춘다', (fieldName) => {
  ...
  describe('검출기 반증', () => {
    ...
    it('feature 모듈에서 판매자 핸들러를 실제로 찾는다(수집기가 비어 있지 않다)', () => {
      ...
      expect(handlerAuth.has('adminDashboardSummary')).toBe(true);
    });
  });
});
