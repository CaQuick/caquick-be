...
// 아래 예외 목록은 P1 진행 중 실측(시작 92 → 현재 45 사이트)이며, 항목이 옮겨질 때마다 줄을 지운다 — 늘어나면 실패한다.
...
const WRITE_EXCEPTIONS: Array<[file: string, model: string, sites: number]> = [
  ...
  ['src/features/admin/repositories/admin.repository.ts', 'Store', 2],
...
describe('모델 소유권 (단일 writer)', () => {
  ...
  it('소유 feature 밖 write는 예외 목록과 정확히 일치한다', () => {
    ...
    expect(found.reduce((sum, [, , n]) => sum + n, 0)).toBe(45);
  });
