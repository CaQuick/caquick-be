...
export class AdminRegionService extends AdminBaseService {
  ...
  async adminCreateRegion(
    ...
    if (parentId !== null && !(await this.repo.isActiveRegionGroup(parentId))) {
      throw domainError('PARENT_REGION_INVALID');
    }
    const slug = input.slug.trim();
    if (await this.repo.existsActiveRegionSlug(slug)) {
      throw domainError('REGION_SLUG_TAKEN');
    }
    const data = {
      parent_id: parentId,
    ...
    if (row === 'slug-taken') throw domainError('REGION_SLUG_TAKEN');
  ...
  async adminUpdateRegion(
    ...
    if (!current) throw domainError('REGION_NOT_FOUND');
    ...
    switch (row) {
      case 'not-found':
        throw domainError('REGION_NOT_FOUND');
      case 'slug-taken':
        throw domainError('REGION_SLUG_TAKEN');
      case 'has-active-children':
        throw domainError('REGION_HAS_ACTIVE_CHILDREN');
      case 'parent-not-active':
        throw domainError('REGION_PARENT_INACTIVE');
      default:
        return toAdminRegionOutput(row);
    }
  }
  ...
  async adminDeleteRegion(
    ...
    if (result === 'not-found') throw domainError('REGION_NOT_FOUND');
    if (result === 'has-stores') throw domainError('REGION_HAS_STORES');
    if (result === 'has-children') {
      throw domainError('REGION_HAS_CHILDREN');
    }
    return true;
  }
