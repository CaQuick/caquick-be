...
/**
 * today-pickup 리스트·픽업 달력/시간 슬롯·주문 생성 재검증이 모두 이 모듈을 소비한다 —
 * 판정 분기를 한쪽만 고치는 규칙 불일치를 구조적으로 차단한다. DI-free 순수 함수만 둔다(시각은 호출부가 주입).
 */

export interface TodaySlotPolicy {
  openMinutes: number;
  /** 미포함 — 슬롯은 픽업 '시각' 포인트라 시작 시각이 close 이전이면 유효하다. */
  closeMinutes: number;
...
/** 슬롯 단위 예약 점유는 capacity가 일(日) 단위 모델이라 표현하지 않는다 — capacity 소진은 호출부에서 매장 자체를 제외한다. */
...
/** Prisma @db.Time(0) 값은 1970-01-01 UTC 기반 Date다. */
...
export interface PickupDayResult {
  ...
  /** reason이 있어도 영업일이면 슬롯을 반환한다(마감 표기용). */
  slots: TodayPickupSlot[];
}

/** 판정 순서: 과거 → 범위 초과 → 특별휴무 → 요일 휴무/영업시간 미설정 → capacity 소진 → 리드타임 반영 잔여 가용 슬롯 없음. */
...
/** 리드타임 컷오프는 절대 시각(now + 리드타임) 기준이라 하루를 넘는 리드타임(최대 7일)도 미래 날짜에 올바르게 적용된다. */
