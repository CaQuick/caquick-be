...
import { MAX_BANNER_TITLE_LENGTH } from '@/features/product/constants/product-admin.constants';
import { MAX_URL_LENGTH } from '@/features/product/constants/product-seller.constants';
import type { AdminBannerListInput } from '@/features/product/dto/inputs/admin-banner-list.input';
import type { AdminCreateBannerInput } from '@/features/product/dto/inputs/admin-create-banner.input';
import type { AdminUpdateBannerInput } from '@/features/product/dto/inputs/admin-update-banner.input';
import { ProductAdminRepository } from '@/features/product/repositories/product-admin.repository';
import { toAdminBannerOutput } from '@/features/product/services/product-admin-banner-mappers.helper';
import type { AdminBannerOutput } from '@/features/product/types/product-admin-output.type';
import {
  AuditActionType,
  AuditTargetType,
  type Banner,
...
export class AdminBannerService extends AdminBaseService {
  constructor(
    ...
    protected readonly repo: ProductAdminRepository,
  ...
  private async validateExposure(final: BannerExposure): Promise<void> {
    ...
    if (final.placement === 'CATEGORY' && final.linkType !== 'CATEGORY') {
      throw new DomainException('CATEGORY_PLACEMENT_REQUIRES_CATEGORY_LINK');
    }
    ...
    switch (final.linkType) {
      ...
      case 'URL':
        if (!final.linkUrl || final.linkUrl.trim().length === 0) {
          throw new DomainException('LINK_URL_REQUIRED');
        }
        return;
      case 'PRODUCT':
        if (!final.linkProductId) {
          throw new DomainException('LINK_PRODUCT_REQUIRED');
        }
        if (!(await this.repo.isProductVisible(final.linkProductId))) {
          throw new DomainException('LINK_PRODUCT_NOT_VISIBLE');
        }
        return;
      case 'STORE':
        if (!final.linkStoreId) {
          throw new DomainException('LINK_STORE_REQUIRED');
        }
        if (!(await this.repo.isStoreVisible(final.linkStoreId))) {
          throw new DomainException('LINK_STORE_NOT_VISIBLE');
        }
        return;
      case 'CATEGORY': {
        if (!final.linkCategoryId) {
          throw new DomainException('LINK_CATEGORY_REQUIRED');
        }
        const categoryType = await this.repo.findVisibleCategoryType(
          final.linkCategoryId,
        );
        if (!categoryType) {
          throw new DomainException('LINK_CATEGORY_NOT_VISIBLE');
        }
  ...
  private assertInputLinkFieldsMatch(
    ...
    if (!allowed[intendedLinkType]) {
      throw new DomainException('LINK_FIELDS_MISMATCH');
    }
  }
}
