 ...
 * publisher/subscriber는 Redis 프로토콜 제약(SUBSCRIBE 중 일반 명령 불가) 때문에 커넥션을 분리한다.
 */
@Global()
@Module({
  providers: [
    {
      ...
      useFactory: (config: ConfigService): RedisPubSub => {
        ...
        const options: RedisOptions = {
          ...
          // 발행은 DB 커밋 이후의 부수효과 — 무한 재시도(null)로 두면 Redis 장애 시 mutation 응답이 매달린다.
          // 짧게 실패시키고 실패 처리는 발행부(try/catch)가 담당한다.
